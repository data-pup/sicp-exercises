...
export const initializeQueue = (inputString:string)
    ...
    const freqQueue = new PriorityQueue<CharFreqRecord>(compareCharFreqRecords);
     ...
     // Return an empty queue if the input string is empty.
    if (isNullOrUndefined(inputString)) {
        return freqQueue;
    }

    // Return an empty queue if the input is empty.
    if (inputString.length === 0) {
        return freqQueue;
    }

    // Split and sort the input string, initialize loop variables.
